...
private extension Ch02_4_ViewLayout_Modifier {
  ...
  struct Example03: View {
    var body: some View {
      Circle()
        .fill(Color.yellow.opacity(0.8)) //투명도 확인, overlay는 굵게/backgound는 흐리게
        .frame(width: 250, height: 250) //크기는 이거 따라가는듯?
        .overlay(Text("Joystick").font(.largeTitle))
        .overlay(Image(systemName: "arrow.up").font(.title).padding().background(Color.red),
                 alignment: .top)
        .overlay(Image(systemName: "arrow.left").font(.title).padding().background(Color.blue),
                 alignment: .leading)
        .overlay(Image(systemName: "arrow.up.right.circle.fill").font(.title), alignment: .topTrailing)
        .background(Image(systemName: "arrow.down").font(.title).padding().background(Color.green),
                    alignment: .bottom)
        .background(Image(systemName: "arrow.right").font(.title).padding().background(Color.pink),
                    alignment: .trailing)
    }
  }
